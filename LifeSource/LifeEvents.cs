    ...
    public class LifeEvents : IScript
    {
        ...
        public void SendCrimes(ShPlayer player, ShPlayer criminal)
        {
            if (LifeManager.pluginPlayers.TryGetValue(criminal, out var pluginPlayer))
            {
                ...
                player.svPlayer.SendOptionMenu($"{player.criminal}'s Criminal Record: ${pluginPlayer.GetFineAmount()}", criminal.ID, crimesMenu, options.ToArray(), new LabelID[] { new LabelID("Get Details", string.Empty) });
            }
        }
